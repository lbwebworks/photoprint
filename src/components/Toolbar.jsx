...
const TILE_SIZE   = 75   // preset tile width & height in px
const TILE_GAP    = 4
...
export default function Toolbar({
  ...
  useEffect(() => {
    function onMove(e) {
      ...
      setPanelWidth(Math.min(maxW, Math.max(MIN_PANEL, startW.current + delta)))
    }
  ...
  // Compute columns that fit in the available preset grid width
  const gridAvailW = panelWidth - PANEL_PAD * 2
  ...
  return (
    <aside
      style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)', width: panelWidth, position: 'relative' }}
      className={`shrink-0 border-r flex flex-col overflow-y-auto ${disabled ? 'opacity-50 pointer-events-none' : ''}`}
    >
